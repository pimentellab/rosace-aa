...
#' Generate Score Line Plot
#'
#'`scoreLn` generates a line plot for visualizing the changes of position-level
#' estimate across positions.
#'
#' @param data Position data frame. Expected to have columns containing information about 
#' position, control amino acid, mutated amino acid, mutation type, and score. 
#' @param savedir Character string specifying the directory to save plots.
#' @param pos.col Column name in `data` for mutation positions. Default is `position`.
#' @param wt.col Column name in `data` for wildtype amino acids. Default is `wildtype`.
#' @param mean.col Column name in `data` for position-level parameters. Default is `phi_mean`.
#' @param sd.col Column name in `data` for error bar of mean parameters. Default is `sigma2_mean`.
#' @param npos Integer specifying the number of positions per subplot. Default is `50`.
#' @param ncol Integer specifying the number of columns of subplots. Default is `1`.
#' @param pos.step Integer specifying the steps between x-axis labels. Default is `5`.
#' @param x.text Numeric value for x-axis text size. Default is `8`.
...
scoreLnplot <- function(data,
  ...
  plot_list <- lapply(1:length(starts), function(i) {
    ...
    # create subplot lineplot
    p <- ggplot(sub_data, aes(x = factor(.data[[pos.col]]))) + 
      geom_point(aes(y = .data[[mean.col]])) +
  #    geom_line(aes(y = .data[[mean.col]])) +
      coord_cartesian(clip = "off") +
      geom_errorbar(aes(y = .data[[mean.col]],
                        ymin = .data[[mean.col]] - .data[[sd.col]],
                        ymax = .data[[mean.col]] + .data[[sd.col]])) +
      scale_x_discrete(breaks =  sub_pos.order[seq(1, length(sub_pos.order), by = pos.step)]) +
      theme(
        plot.margin = unit(c(2, 1, 1, 1), "lines"),
        axis.title.y = element_text(margin = margin(t = 3, r = 0, b = 0, l = 0)),
...
scoreLnplot_multi <- function(data,
                        savedir,
                        pos.col = "position",
                        wt.col = "wildtype",
                        mean.col = "phi_mean",
  ...
  # value
  ymax <- max(data[[mean.col]] + data[[sd.col]])
  ymin <- min(data[[mean.col]] - data[[sd.col]])
  ...
  plot_list <- lapply(1:length(starts), function(i) {
    ...
    # create subplot lineplot
    p <- ggplot(sub_data, aes(x = factor(.data[[pos.col]]), color = .data[[group.col]])) + 
      geom_point(aes(y = .data[[mean.col]])) +
      geom_line(aes(y = .data[[mean.col]], group = .data[[group.col]])) +
      geom_hline(yintercept = 0) + 
      coord_cartesian(clip = "off", ylim = c(ymin, ymax)) +
      geom_errorbar(aes(y = .data[[mean.col]],
                        ymin = .data[[mean.col]] - .data[[sd.col]],
                        ymax = .data[[mean.col]] + .data[[sd.col]]), alpha = 0.2) +
      scale_x_discrete(breaks =  sub_pos.order[seq(1, length(sub_pos.order), by = pos.step)]) +
      theme_classic() +
      theme(
        plot.margin = unit(c(2, 1, 1, 1), "lines"),
