...
If you have cloned the git repository locally, navigate to the `rosace-aa` folder and type:
...
There are a few options to run Rosace model:
Model 1
1) pos.col: provide position column
2) ctrl.col, ctrl.name, stop.col, stop.name: provide control or/and stop column (only useful when position column is provided).
Model 2
...
The functional score represents the slope of the linear regression applied to normalized counts across time points, which serves as a measure of cell growth. A positive score indicates the mutation has a a gain of function (GOF) effect, while a negative score indicates a loss of function (LOF) effect. Similar to Rosace, the RosaceAA model learns a distribution of scores and reports the mean for each variant. 
